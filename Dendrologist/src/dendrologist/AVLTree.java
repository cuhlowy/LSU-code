...
/*===> BEGIN: Augmented public methods <===*/   
   ...
   public ArrayList<E> getChildren(E entry) throws AVLTreeException
   {
      //Implement this method
	    ...
	    while (currentNode != null) {
	        ...
	        if (cmpResult == 0) {
	            if (currentNode.left != null) {
	                children.add(currentNode.left.data);
	            } else {
	                children.add(null);
	            }
	            if (currentNode.right != null) {
	                children.add(currentNode.right.data);
	            } else {
	                children.add(null);
	            }
	            return children;
	   ...
	   // return children;
	    throw new AVLTreeException("AVL Tree Exception: entry not found in getChildren()");
  }
   ...
   public E getParent(E entry) throws AVLTreeException
   {
	   Node parentNode = null;
	    ...
	    while (currentNode != null) {
	        ...
	        if (cmpResult == 0) {
	            if (parentNode == null) {
	                throw new AVLTreeException("AVL Tree Exception: root has no parent in getParent()");
	            }
	    ...
	    throw new AVLTreeException("AVL Tree Exception: entry not found in getParent()");
	}
   ...
   public int ancestors(E entry) throws AVLTreeException
   {
	   ...
	   int numAncestors =0;
	    ...
	    if (currentNode==null) {
	    	throw new AVLTreeException("AVL Tree Exception: entry not found in ancestors()");
	    }
	    return numAncestors;
   ...
   public int descendants(E entry) throws AVLTreeException
   {
	...
	int des;
	   ...
	   while (tmp!=null){
		   ...
		   int ret =cmp.compare(entry, tmp.data);
		   ...
		   if (ret<0) {
			   ...
			   des = countDesc(tmp.left) + countDesc(tmp.left);
		   return des;
		   }
			   
	   }
	   throw new AVLTreeException("Node not found");
	}
   ...
   public boolean isFibonacci()
   {
	   ...
	   {
	   Node node = q.poll();
	   int current = height(node);
	   if(last != 1 && current != last + 1){ return false;}
	   last = current;
